...
void _draw_combobox_button(void *w_, void* user_data) {
    ...
    int wa = width/1.2;
    int h = height/2.5;
    cairo_move_to(w->crb, wa-2+offset, h+2+offset);
    cairo_line_to(w->crb, wa-5+offset, h+8+offset);
    cairo_line_to(w->crb, wa-8+offset, h+2+offset);
    cairo_line_to(w->crb, wa-2+offset, h+2+offset);
    cairo_fill(w->crb);
